...
import { asCurrency, CURRENCIES, currencySymbol, floorFor, formatMinor, formatRate, NUMBER_TYPES, NUMBER_TYPE_LABELS, surchargePercent, type CarrierMonthly } from "@/lib/billing/pricing";
    ...
    <form action={submit} className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <label className="space-y-1.5">
          ...
          {locked && <input type="hidden" name="currency" value={currency} />}
          <select name={locked ? undefined : "currency"} value={currency} disabled={locked} onChange={(e) => setCurrency(asCurrency(e.target.value))} className="input">
            {CURRENCIES.map((c) => (
          ...
          <span className="block text-xs text-slate-500">{locked ? "Fixed once published: Stripe prices are single-currency." : "Every amount below is in this currency. Fixed once published."}</span>
        </label>
      </div>
      ...
      <div key={plan ? plan.id : currency} className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-1.5">
              ...
              <span className="block text-xs text-slate-500">
                Pooled across forwarded, inbound and browser minutes. Each costs about {formatRate(floor.perMinute, currency)} of carrier time, so hosting must cover them. 0 = pay per minute from
                the first minute.
              </span>
            </label>
          </div>
        ...
        <fieldset className="space-y-3">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label className="space-y-1.5">
              <span className="text-sm font-medium">Per minute ({formatRate(1, currency).replace(/^1/, "")})</span>
            ...
            <label className="space-y-1.5">
              <span className="text-sm font-medium">0800 inbound per minute ({formatRate(1, currency).replace(/^1/, "")})</span>
